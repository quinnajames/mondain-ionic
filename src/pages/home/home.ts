...
export class HomePage {
  ...
  goToQuiz() {
    // let requery = this.getAnagramList(this.db, this.inputWordLength, undefined, -(-this.inputListSize), -(-this.inputStartPos), true);
    // let quizList = [];
    // requery.subscribe(snapshots => {
    //   snapshots.forEach(snapshot => {
    //     quizList.push(snapshot.key);
    //     console.log(snapshot.key);
    //   })
    // })
